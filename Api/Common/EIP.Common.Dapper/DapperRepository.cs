...
namespace EIP.Common.Dapper
{
    ...
    public class DapperRepository<TEntity> : IDapperRepository<TEntity> where TEntity : class
    {
        ...
        public virtual async Task<bool> DeleteByIdsAsync(string ids) 
        {
            ...
            var deleted = await Connection.ExecuteAsync(queryResult.GetSql(), queryResult.Param) > 0;
            return deleted;
        }
        ...
        {
            return await Connection.QueryAsync<TEntity>(sqlQuery.GetSql(), sqlQuery.Param, transaction);
        }
