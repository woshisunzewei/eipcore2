...
function GooFlow(bgDiv, property) {
    ...
    if (property.haveHead) {
        ...
        for (var x = 0; x < property.headBtns.length; ++x) {
            ...
            tmp += "<a href='javascript:void(0)' data-toggle='tooltip' class='GooFlow_head_btn'" + titleText + "><i class='ico_" + property.headBtns[x] + "'></i></a>"
        }
        tmp += "</div>";
        this.$head = $(tmp);
    ...
    if (property.haveTool) {
        ...
        this.$tool.append("<div style='margin-bottom:4px'><span/><span/><span/></div>"
            + "<a  data-toggle='tooltip' href='javascript:void(0)'" + titleCursor + " type='cursor' class='GooFlow_tool_btndown' id='" + this.$id + "_btn_cursor'><i class='ico_cursor'/></a>"
            + "<a  data-toggle='tooltip' href='javascript:void(0)'" + titleDirect + " type='direct' class='GooFlow_tool_btn' id='" + this.$id + "_btn_direct'><i class='ico_direct'/></a>"
            + (property.haveDashed ? "<a  data-toggle='tooltip' href='javascript:void(0)'" + titleDashed + " type='dashed' class='GooFlow_tool_btn' id='" + this.$id + "_btn_dashed'><i class='ico_dashed'/></a>" : "")
        );
        if (property.toolBtns && property.toolBtns.length > 0) {
            ...
            for (var i = 0; i < property.toolBtns.length; ++i) {
                ...
                tmp += "<a  data-toggle='tooltip' href='javascript:void(0)'" + titleText + " type='" + property.toolBtns[i] + "' id='" + this.$id + "_btn_" + tmpType + "' class='GooFlow_tool_btn'><i class='ico_" + property.toolBtns[i] + "'/></a>";//加入自定义按钮
            }
        ...
        if (property.haveGroup) {
            ...
            this.$tool.append("<span/><a  data-toggle='tooltip'  href='javascript:void(0)'" + titleGroup + " type='group' class='GooFlow_tool_btn' id='" + this.$id + "_btn_group'><i class='ico_group'/></a>");
        }
        toolWidth = 31;
        this.$nowType = "cursor";
        //绑定各个按钮的点击事件
    ...
    this.$bgDiv.append("<div  data-toggle='tooltip' class='GooFlow_work' style='" + (property.haveHead ? "top:28px;" : "") + (property.haveTool ? "left:34px" : "") + "'></div>");
    this.$workArea = $("<div  data-toggle='tooltip' class='GooFlow_work_inner' style='width:" + width + "px;height:" + height + "px'></div>")
    ...
    if (this.$editable) {
        ...
        this.$workArea.on("click", { inthis: this }, function (e) {
            ...
            This.addNode(UtilNewGuid(), { name: "node_" + This.$max, left: X, top: Y, type: This.$nowType });
            This.$max++;
        });
        //划线或改线时用的绑定
...
GooFlow.prototype = {
    ...
    initGroup: function (width, height) {
        ...
        this.$group.mouseup({ inthis: this }, function (e) {
            ...
            if (e.data.inthis.$ghost.css("display") == "none") {
                ...
                e.data.inthis.addArea(UtilNewGuid(),
    ...
    initWorkForNode: function () {
        ...
        this.$workArea.delegate(".GooFlow_item > .span", "dblclick", { inthis: this }, function (e) {
            var This = e.data.inthis;
            var type = $('.item_focus').hasClass('item_round');
            if (type) {
                openNode(This);
            }
        });
