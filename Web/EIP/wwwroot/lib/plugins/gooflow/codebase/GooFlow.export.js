...
GooFlow.prototype.exportDiagram=function(fileName) {
    ...
    var thisId=this.$id;
    $("body").append('<div id="'+thisId+'_export" style="position:absolute;top:0;left:0;z-index:-1;width:0px;height:0px;overflow:hidden">'
    ...
    //1.先COPY节点和区块的内容
    var inner = $("#"+thisId).find(".GooFlow_work_inner");
    var divCanvas = $("#"+thisId+"_export").children("div:eq(0)");
    ...
    html2canvas(divCanvas[0], {
        ...
        onrendered: function(canvas) {
            ...
            var strSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="'+width+'" height="'+height+'">'
                ...
                +'}</style></defs>' + window.$("<svg>").append(window.$("#draw_"+thisId).clone()).html() +'</svg>'; //COPY连线内容
            var image = null;
